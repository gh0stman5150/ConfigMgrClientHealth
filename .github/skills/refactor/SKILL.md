...
- **Large functions:** split long `Test-*` functions into detection and remediation functions. Keep the original function name and parameters as the entry point that the Process block calls. `Test-Service`, `Test-ConfigMgrClient`, and `Test-DNSConfiguration` are worked examples. Before moving code into a helper, check which variables its callees read from the caller's scope (for example, `Resolve-Client` used to read `$Uninstall` that way) and pass them as parameters instead.
